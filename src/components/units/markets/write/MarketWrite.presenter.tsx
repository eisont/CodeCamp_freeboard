...
import { contentsErrorState, LatState, LngState, nameErrorState, priceErrorState, remarksErrorState } from '../../../../commons/store';
...
const MarketWriteUI = (props: IMarketWriteUIProps) => {
  ...
  return (
    <S.Wrapper>
      ...
      <S.InputWrapper>
        <S.Label>상품명</S.Label>
        <S.Subject type='text' placeholder='(필수) 상품명을 작성해주세요.' onChange={props.onChangeName} />
        <S.Error>{nameError}</S.Error>
      </S.InputWrapper>
      {/* 한줄요약 */}
      <S.InputWrapper>
        <S.Label>한줄요약</S.Label>
        <S.Subject type='text' placeholder='(필수) 한줄요약을 작성해주세요.' onChange={props.onChangRemarks} />
        <S.Error>{remarksError}</S.Error>
      </S.InputWrapper>
      {/* 상품설명 */}
      <S.InputWrapper>
        <S.Label>상품설명</S.Label>
        <ReactQuill onChange={props.onChangeContents} style={{ width: '996px', height: '300px', background: '#fff' }} placeholder='(필수) 상품설명을 작성해주세요.' />
        <S.Error>{contentsError}</S.Error>
      </S.InputWrapper>
      {/* 판매가격 */}
      <S.InputWrapper>
        <S.Label>판매가격</S.Label>
        <S.Subject type='number' placeholder='(필수) 판매가격을 작성해주세요.' onChange={props.onChangePrice} />
      ...
      <S.TagWrapper>
        ...
        <S.TagRBox>
          <S.Subject type='text' placeholder='(필수) 태그를 선택해주세요' onKeyUp={props.onChangeTags} />
          <S.Span>
      ...
      <S.InputWrapper>
        ...
        <S.ZonecodeWrapper>
          ...
          {props.isOpen && (
            <Modal visible={props.isOpen} onOk={props.onClickAddressSearch} onCancel={props.onClickAddressSearch} width={800} closeIcon={<SmileOutlined />}>
              <DaumPostcode onComplete={props.onCompleteAddressSearch} />
            </Modal>
          )}
        </S.ZonecodeWrapper>
        <S.Addressbasic readOnly value={props.address || props.data?.createUseditem.useditemAddress?.address || '(선택) 주소를 선택해주세요.'} />
        <S.AddressDetail
          placeholder='(선택) 상세 주소를 입력해주세요.'
          onChange={props.onChangeAddressDetail}
          defaultValue={props.addressDetail || props.data?.createUseditem.useditemAddress?.addressDetail || ''}
        />
      </S.InputWrapper>
