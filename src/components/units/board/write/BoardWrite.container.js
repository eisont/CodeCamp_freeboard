...
import { CREATE_BOARD, FETCH_LOGGEDIN_USER, UPDATE_BOARD } from './BoardWrite.queries';
...
const BoardWrite = (pr) => {
  ...
  // input 변수
  const [zonecode, setZonecode] = useState(pr.fetchBoardData?.fetchBoard?.boardAddress?.zipcode || '');
  const [address, setAddress] = useState(pr.fetchBoardData?.fetchBoard?.boardAddress?.address || '');
  ...
  const onClickUpdate = async (data) => {
    ...
    const ContentsCompare = data?.contents === pr.fetchBoardData?.fetchBoard?.contents;
    const zipcodeCompare = zonecode === pr.fetchBoardData?.fetchBoard?.boardAddress?.zipcode;
    const AddressCompare = address === pr.fetchBoardData?.fetchBoard?.boardAddress?.address;
    const AddressDetailCompare = data?.addressDetail === pr.fetchBoardData?.fetchBoard?.boardAddress?.addressDetail;
    const YoutubeUrlCompare = data?.youtubeUrl === pr.fetchBoardData?.fetchBoard?.youtubeUrl;

    if (TitleCompare && ContentsCompare && zipcodeCompare && AddressCompare && AddressDetailCompare && YoutubeUrlCompare && ImageCompare) {
      alert('수정한 내용이 없습니다.');
    }
    ...
    if (!zipcodeCompare || !AddressCompare || !AddressDetailCompare) {
      ...
      if (!AddressDetailCompare) updateBoardInput.boardAddress.addressDetail = data?.addressDetail;
    }
