      ...
      <S.Contents>
        <S.Header>
          <S.UserInformation>
            {pr.data?.fetchBoard?.user?.picture !== undefined ? (
              <S.UserProfilePhoto src={`https://storage.googleapis.com/${pr.data?.fetchBoard?.user?.picture}`} />
            ) : (
              <Profilesvg margin='0 16.67px 0 0' width='47' height='47' fill='#bdbdbd' />
            )}
            <div>
              <S.UserName>{pr.data?.fetchBoard?.writer}</S.UserName>
              <S.CreatedAt>Date: {getDatecomma(pr.data?.fetchBoard?.createdAt)}</S.CreatedAt>
            </div>
          </S.UserInformation>
          <S.HeaderButtons>
            ...
            <Tooltip
              placement='top'
              title={pr.data?.fetchBoard?.boardAddress ? `${pr.data?.fetchBoard.boardAddress?.address} ${pr.data?.fetchBoard.boardAddress?.addressDetail}` : '등록한 주소가 없습니다.'}
            >
          ...
          {typeof window !== 'undefined' && (
            <S.SectionContent
              dangerouslySetInnerHTML={{
                __html: DOMPurify.sanitize(String(pr.data?.fetchBoard?.contents)),
              }}
            />
          )}

          <S.SectionVideoBox>
            {pr.data?.fetchBoard.youtubeUrl !== null && <SectionVideoURL url={pr.data?.fetchBoard.youtubeUrl} width={486} height={240} muted={true} playing={true} />}
          </S.SectionVideoBox>
        </S.Body>
        ...
        <S.LikeButtonBox>
          ...
          <S.DisLikeButton onClick={pr.ondislikeCount}>
            ...
            <S.DisLikeNumber>{pr.data?.fetchBoard.dislikeCount}</S.DisLikeNumber>
          </S.DisLikeButton>
        </S.LikeButtonBox>
      </S.Contents>
