...
import { Closesvg, Pencilsvg, Profilesvg, Questionsvg } from '../../../../commons/styles/Iconsvg';
...
const MarketCommentListUIItem = (props: IMarketCommentListUIItemProps) => {
  ...
  return (
    <>
      {isEdit ? (
        <MarketCommentWrite isEdit={true} setIsEdit={setIsEdit} commentID={props.commentel?._id} />
      ) : (
        <S.ItemWrapper>
          <S.FlexBox>
            {props.commentel?.user?.picture === null ? (
              ...
              <S.Avatar src={props.commentel?.user?.picture?.includes('data:image') ? props.commentel?.user?.picture : `https://storage.googleapis.com/${props.commentel?.user?.picture}`} />
            )}

            <S.TitleBox>
              <S.UserName>{props.commentel?.user?.name}</S.UserName>
              <S.Contents>{props.commentel?.contents}</S.Contents>
              <S.CreateAt>{getDatecomma(props.commentel?.createdAt)}</S.CreateAt>
            </S.TitleBox>
          </S.FlexBox>

          <S.BtBox>
            <S.Bt onClick={onClickCreateAnswer}>
              <Questionsvg margin='0 16px 0 0' width='20' heigth='20' fill='#bdbdbd' />
            </S.Bt>
            <S.Bt onClick={onClickUpdate}>
              <Pencilsvg margin='0 16px 0 0' width='18' heigth='18' fill='#bdbdbd' />
            </S.Bt>
      ...
      {isEditSub && <MarketCommentAnswerWrite isEditSub={true} setIsEditSub={setIsEditSub} commentID={props.commentel?._id} />}
      <MarketAnswerList isEditSub={isEditSub} setIsEditSub={setIsEditSub} commentID={props.commentel?._id} />
      <S.Hr />
    </>
  );
};
